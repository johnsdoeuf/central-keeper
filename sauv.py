...
def verifie_arbre(config, branche):
	...
	for rep in os.scandir(config[dest]):
		...
		if rep.is_dir():
			...
			if rep.name == temp:
				logger.warning(
					"{}-Un répertoire {} est trouvé, l'ancienne sauvegarde ne s'est pas bien terminée".format(
						config.name, temp))
				continue
			...
			if cas == 3:
				try:
					...
					logger.warning(
						"{}-la troisième partie du répertoire '{}' n'est pas un entier; il est ignoré".format(
	...
	for incr in iterateur:
		if incr.date <= old_date:
			logger.warning(
				"un increment de sauvegarde est mal daté ou pas dans le bon niveau :{}, il est supprimé ".format(incr))
			try:
				arbre[0].remove(incr)
			except ValueError:
				pass
	...
	if not comparaison_arbre(arbre, branche):
		logger.warning(
			"{}-Le fichier d'historique ne correspond pas au répertoires trouvé avec 'verifie_arbre'".format(
				config.name))
	...
	for lnv, lenr in zip(branche1, branche2):
		...
		for sauv_nv, sauv_enr in zip(lnv, lenr):
			...
			if sauv_nv.date != sauv_enr.date:
				logger.warning(
					"Les sauvegardes en mémoire et sur disques sont différentes: date {} != {}".format(sauv_nv.date,
																									   sauv_enr.date))
				return False
			# compare les chemins
			if sauv_nv.chemin != sauv_enr.chemin:
				logger.warning(
					"Les sauvegardes en mémoire et sur disques sont différentes: chemin {} != {}".format(sauv_nv.chemin,
																										 sauv_enr.chemin))
				return False
...
def regroupe(cons, per_suiv, arbre, offset_arbre, ref):
	...
	while arbre[offset_arbre][-1 - saute].date < ref - datetime.timedelta(days=cons) and len(
			arbre[offset_arbre]) >= 2 + saute:
	...
	for niv in range(3, 0, -1):
		...
		for sav in range(len(arbre[niv - 1]) - 1, -1, -1):
			...
			if objectif < taille_arbre(arbre):
				...
				if nbrsauv <= 0:
					logger.warning(
						"{}-'{}' est la dernière sauvegarde, elle ne peut être effacée ".format(config.name, rep))
					...
					try:
						...
						logger.warning(
							"{}-Erreur lors de l'effacement du répertoire '{}'\n{}".format(config.name, rep.chemin,
																						   exception))
						...
						if ecart_cons - limite_cons * 0.75 < 0:
							...
							logger.error(
								"{}-Le répertoire '{}' aurait du être conservé,\n le quota ne permet pas de respecter 75% du critère de conservation CONSERVATION{}".format(
									config.name, rep.chemin, niv))
						else:
							if ecart_cons - limite_cons < 0:
								# message si la conservation est réduite entre 75% et 100%
								logger.warning(
									"{}-Le répertoire '{}' aurait du être conservé,\n le quota ne permet pas de respecter le critère de conservation CONSERVATION{}".format(
										config.name, rep.chemin, niv))
...
def init_logging(fichier):
	...
	if not os.path.exists(fichier):
		...
		logger.warning("fichier de configuration {0} ne se trouve pas dans {1}".format(conf_log, os.path.split(
...
def monte(sens, config):
	...
	if mnt[sens] in config:
		...
		if umnt[sens] in config:
			...
			logger.warning(
				"{}-Une commande de montage a été utilisée sans que le démontage soit configuré".format(config.name))
...
def init_args():
	...
	parser.add_argument('-f', '--force', action="store_true",
						help='force la sauvegarde, ne tient pas compte du paramètre PERIODE1')
...
def analyse_retour_pour_bilan(lignes, sauv, md5, cli):
	...
	for l in lignes:
		result = regex1.search(l)
		if result:
			cli.stat[bl_voltransfere] = int(re.sub(",", "", result.group("sent"))) + int(
				re.sub(",", "", result.group("received")))
			logger.debug("Ligne 'sent received' trouvée ")
